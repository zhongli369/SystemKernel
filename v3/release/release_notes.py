"""
Release Notes — Generate SystemKernel v3.0 release notes.

Phase 5F: Release-grade documentation of what v3.0 is, what it includes,
and what is intentionally excluded. No new capabilities.
"""
...
RELEASE_CODENAME = "Deterministic Kernel"


def generate_release_notes(v3_root: Optional[str] = None) -> str:
    """Generate SystemKernel v3.0 release notes as markdown."""
    if v3_root is None:
    ...
    mem_report = _read_json(os.path.join(exports_dir, "memory_system_report.json"))
    cb_report = _read_json(os.path.join(exports_dir, "complexity_budget_report.json"))

    purity = kernel_report.get("purity_score", "?")
    removable = mem_report.get("verdicts", {}).get("removability", "?")
    cb_verdict = cb_report.get("verdict", {}).get("verdict", "?")

    # Count tests
    tests_dir = os.path.join(v3_root, "tests")
    test_count = _count_tests(tests_dir)

    # Count modules
    module_count = _count_modules(v3_root)

    # External registry
    registry = _read_json(os.path.join(exports_dir, "external_tool_registry.json"))
    reg_count = len(registry.get("entries", []))

    notes = f"""# SystemKernel v3.0 — Release Notes
...
| Complexity gate | {cb_verdict} |
| Python modules | {module_count} |
| Test functions | {test_count} |
| External tools evaluated | {reg_count} |
| CLI commands | 12 |
...
*SystemKernel v3.0 "{RELEASE_CODENAME}" — {RELEASE_DATE}*
*Feature freeze. No new runtime capabilities.*
"""

    # Compute hash after notes are fully built
    notes += f"\n\n<!-- Release notes hash: {_compute_hash(notes)} -->\n"
    return notes


def write_release_notes(notes: str, path: str) -> str:
    """Write release notes to a markdown file. Returns absolute path."""
...
def _count_tests(tests_dir: str) -> int:
    import ast
    total = 0
...
def _count_modules(v3_root: str) -> int:
    count = 0
    for sub in ("kernel", "memory", "quality", "intake", "cli", "release"):
        d = os.path.join(v3_root, sub)
        if os.path.isdir(d):
