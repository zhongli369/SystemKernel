"""
Quality Intelligence Policy — v4.1.

Defines policies governing which quality intelligence providers are
...
class QualityIntelligencePolicy:
    """Policy governing quality intelligence provider operations.
...
def default_quality_intelligence_policy() -> QualityIntelligencePolicy:
    """Return the default quality intelligence policy.

    Methodology-based providers are allowed (analysis-only).
    No LLM, no external services.
    Suggested weight for decision fusion: 0.6.
    """
    from v3.external.quality_intelligence import ALL_SIGNAL_TYPES

    policy = QualityIntelligencePolicy(
...
def block_provider_reason(provider, policy: QualityIntelligencePolicy) -> str:
...
def validate_provider_against_policy(
    ...
    """Validate a QualityIntelligenceProvider against the policy."""
...
def validate_result_against_policy(
    ...
    """Validate a QualityIntelligenceResult against the policy."""
    from v3.external.quality_intelligence import QualityIntelligenceResult

    if not isinstance(result, QualityIntelligenceResult):
        return False, f"Expected QualityIntelligenceResult, got {type(result).__name__}"
